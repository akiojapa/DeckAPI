...
class deckService {
     ...
     async getCommander(commander: string) { 
        ...
        const response = await axios.get(`${scryfall}/cards/named?exact=${commander}`);
     ...
     async getCardsByColor(colors:Array<string>, set:string) {
        ...
        const response = await axios.get(`${scryfall}/cards/search?q=colors:${colors?.join(' ')}&unique=cards&order=random&lang=en&page=1&set=${set}`)
    ...
    async createDeck() {
        ...
        writeFile('deck.json', jsonData, (err) => {
            if(err){
                throw err.message;
            }
            else{
